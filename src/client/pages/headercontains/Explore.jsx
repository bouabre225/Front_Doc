...
const Explore = () => {
    ...
    const fetchAnnonces = useCallback(async () => {
        ...
        try {
            ...
            if (searchQuery.trim()) {
                if (sortBy === 'prix_asc')
                    filtered.sort((a, b) => Number(a.prix_vendeur) - Number(b.prix_vendeur));
                if (sortBy === 'prix_desc')
                    filtered.sort((a, b) => Number(b.prix_vendeur) - Number(a.prix_vendeur));
                if (selectedCategory !== 'Toutes') {
                    filtered = filtered.filter((a) =>
                        a.categorie?.toLowerCase().includes(selectedCategory.toLowerCase()),
                    );
                }
                if (selectedCondition !== 'Tous') {
                    filtered = filtered.filter((a) => {
    ...
    const hasActiveFilters =
        selectedCategory !== 'Toutes' || selectedCondition !== 'Tous' || searchQuery;
