            ...
            <div className="overflow-hidden transition-all duration-300 bg-white border border-gray-100 shadow-sm rounded-2xl hover:shadow-xl group hover:-translate-y-1">
                <div className="relative overflow-hidden h-52">
                    {annonce.images?.[0] ? (
                        ...
                        <div className="flex items-center justify-center w-full h-full bg-gray-100 text-5xl">
                            🏥
                        </div>
                    )}
                    ...
                    {annonce.etat && (
                        <span
                            className={`absolute top-3 left-3 px-2.5 py-1 text-xs font-bold rounded-lg capitalize ${conditionStyle(annonce.etat)}`}
                        >
                            {annonce.etat}
                        </span>
                    )}
                        ...
                        <Link to={`/categories/${key}`}>
                            <div className="relative overflow-hidden transition-all duration-300 bg-white border border-gray-100 shadow-sm cursor-pointer group rounded-2xl hover:shadow-xl hover:-translate-y-1">
                                <div className="relative h-40 overflow-hidden">
                                    ...
                                    />
                                    <div
                                        className={`absolute inset-0 bg-gradient-to-br ${cat.color} opacity-70`}
                                    />
...
const CategoryDetail = ({ slug, category }) => {
    ...
    const displayed = annonces
        .filter((a) => {
            const qMatch = searchQ ? a.titre?.toLowerCase().includes(searchQ.toLowerCase()) : true;
            const etatMatch = etatFilter === 'tous' ? true : a.etat?.toLowerCase() === etatFilter;
        ...
        const fetchCounts = async () => {
            try {
                ...
                Object.entries(data).forEach(([categorie, total]) => {
                    ...
                    const key = Object.keys(CATEGORIES_CONFIG).find(
                        (k) => CATEGORIES_CONFIG[k].name.toLowerCase() === catNorm,
                    );
                    if (key) c[key] = total;
                });
