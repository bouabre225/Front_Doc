...
const Header = () => {
  ...
  // ─── Compteur messages + WebSocket Echo ──────────────────────────────────
  ...
  useEffect(() => {
    ...
    // ✅ Echo pour les messages en temps réel
  ...
  // ─── Écoute les lectures de notifs depuis NotificationsPage ─────────────
  useEffect(() => {
    // ✅ Quand on lit une notif dans NotificationsPage, elle dispatch un event
          ...
          <div className='flex items-center justify-between text-xs'>
            ...
            <div className='items-center hidden gap-4 md:flex'>
              ...
              <div className='relative' ref={langRef}>
                <button onClick={() => setLangOpen(!langOpen)} className='text-gray-600 hover:text-[#1DBF73] transition-colors font-medium flex items-center gap-1'>
                  <span>{t.flag}</span>
                  <span>{t.name}</span>
                      ...
                      <div className='py-1 overflow-y-auto max-h-72'>
                        {langList.map((lang) => (
                          <button key={lang.code} onClick={() => { setCurrentLang(lang.code); setLangOpen(false); }}
                            className={`w-full flex items-center gap-2 px-3 py-2 text-xs hover:bg-gray-50 transition-colors ${currentLang === lang.code ? 'bg-[#1DBF73]/10 text-[#1DBF73] font-semibold' : 'text-gray-700'}`}>
            ...
            <div className='items-center hidden gap-1 md:flex'>
              ...
              {currentUser ? (
                <div className='relative' ref={userMenuRef}>
                  <motion.button onClick={() => setUserMenuOpen(!userMenuOpen)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                    ...
                    <span className='text-sm font-semibold text-gray-700 max-w-[80px] truncate'>{currentUser.nom?.split(' ')[0]}</span>
                  ...
                  <AnimatePresence>
                    {userMenuOpen && (
                      <motion.div initial={{ opacity: 0, y: -8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.95 }}
                        transition={{ duration: 0.15 }} className='absolute right-0 mt-2 w-56 bg-white border border-gray-100 shadow-xl rounded-2xl overflow-hidden z-50'>
                          ...
                          <div className='flex items-center gap-3'>
                            ...
                            <div className='min-w-0'>
                              ...
                              <span className={`inline-block mt-0.5 px-2 py-0.5 text-xs font-semibold rounded-full ${
                                currentUser.role === 'vendeur' ? 'bg-[#09B1BA]/10 text-[#09B1BA]' :
                        ...
                        <div className='py-1'>
                          ...
                          {currentUser.role === 'vendeur' && (
                            <Link to='/publish-equipment' onClick={() => setUserMenuOpen(false)} className='flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 hover:text-[#1DBF73] transition-colors'>
                              <Settings className='w-4 h-4' /> Publier une annonce
                            </Link>
                          )}
                        </div>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <form onSubmit={handleMobileSearch} className='mb-4'>
              <div className='relative'>
                <input type='text' value={mobileSearchQuery} onChange={(e) => setMobileSearchQuery(e.target.value)}
                  placeholder={t.searchPlaceholder} style={{ fontSize: '16px' }}
              ...
              <div className='grid grid-cols-3 gap-1 px-2'>
                {langList.map((lang) => (
                  <button key={lang.code} onClick={() => setCurrentLang(lang.code)}
                    className={`flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs transition-colors ${currentLang === lang.code ? 'bg-[#1DBF73]/10 text-[#1DBF73] font-semibold' : 'text-gray-600 hover:bg-gray-100'}`}>
                    <span>{lang.flag}</span>
